...
```python
import timm_new
model = timm_new.create_model('vit_base_patch16_224', pretrained=True)
model.eval()
```
...
```python
model = timm_new.create_model('vit_base_patch16_224', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
