...
```python
import timm_new
model = timm_new.create_model('gluon_seresnext101_32x4d', pretrained=True)
model.eval()
```
...
```python
model = timm_new.create_model('gluon_seresnext101_32x4d', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
