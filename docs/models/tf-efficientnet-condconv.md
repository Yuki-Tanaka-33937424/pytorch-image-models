...
```python
import timm_new
model = timm_new.create_model('tf_efficientnet_cc_b0_4e', pretrained=True)
model.eval()
```
...
```python
model = timm_new.create_model('tf_efficientnet_cc_b0_4e', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
