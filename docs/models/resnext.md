...
```python
import timm_new
model = timm_new.create_model('resnext101_32x8d', pretrained=True)
model.eval()
```
...
```python
model = timm_new.create_model('resnext101_32x8d', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
