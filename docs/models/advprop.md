...
```python
import timm_new
model = timm_new.create_model('tf_efficientnet_b0_ap', pretrained=True)
model.eval()
```
...
```python
model = timm_new.create_model('tf_efficientnet_b0_ap', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
