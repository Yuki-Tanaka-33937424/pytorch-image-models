...
```python
import timm_new
model = timm_new.create_model('wide_resnet101_2', pretrained=True)
model.eval()
```
...
```python
model = timm_new.create_model('wide_resnet101_2', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
